...
The collection factory appear out of necessity. Our blocking points at that point were that we needed to be able to programmatically and easily deploy new contracts while allowing future API integration within the Sandbox ecosystem.

One of the first things we needed to decide was which way we go:
...
We realized early on, due to painful past experiences, that having one contract address for several collections will lead to a plethora of issues down to road, so we scratched that right from the start. We went with the second option and started looking into proxy factories and management solutions.

## Factory overview

The collection factory, composed of `CollectionFactory` and adjacent contracts, was designed with the KISS principle in mind while also taking in consideration solid security practices. We leveraged the beacon proxy pattern to achieve a one step upgrade to all grouped collections.

We also deployed our own custom proxy implementation to allow for that odd, but still valuable, case where a custom implementations is needed to fit into the puzzle.
...
Each `BeaconProxy` contract holds the address of the `UpgradableBeacon` contract and from that `UpgradableBeacon` it gets the address of the implementation to which to point to. `BeaconProxy` 1 to N all are deployed using the same `UpgradableBeacon` contract, as such they all point to the `Implementation` A contract. You can launch as many proxies as you want, cheap, and get the same effect.

If you want to upgrade the underlying implementation of each proxy all that is needed is to change the address that UpbradableBeacon points to, in a single transaction, and you are done.
...
With the normal beacon proxy if you wish to change the implementation of one specific proxy you could not do that without also upgrading all the other proxies that point to it. This presented a problem as our business logic required us to permit individual proxies (collections) to be modified.

To resolve this issue we created the `CollectionProxy`, an extension of the normal `BeaconProxy` but with the added feature of being able to change the `UpgradableBeacon` contract it points to. With this, if for the `BeaconProxy` #3, now `CollectionProxy` #3, we wanted to change the implementation it points to `Implementation` B we simply change it to point to a `UpgradableBeacon` #2, different from the first which points to this implementation.
...
Both the `UpgradableBeacon` contract and our custom `CollectionProxy` contract have a privileged admin role that is responsible with changing the addresses pointed to in each case.
...
The only step that is not done through the `CollectionFactory` contract is the deployment of a new implementation contract which would not of made sense.

## Technical description of the factory smart contracts

### `CollectionProxy`

The `CollectionProxy` contract is an extension of the OpenZeppelin's `BeaconProxy` implementation that supports having an admin (owner equivalent) that can change the beacon to which this proxy points to. Initial admin is set to the deployer. [[Source code here](https://github.com/thesandboxgame/sandbox-smart-contracts/blob/feat/new-avatar-contract-audit/packages/avatar/contracts/proxy/CollectionProxy.sol)]

One thing to note is that, it is generally a bad idea to add functions in the proxy itself, because any call to an underlying implementation function that has the same sighash with a function from the proxy will actually be executed by the proxy and not the implementation. We know this and accepted the limitations that comes with it. We thoroughly and obsessively documented this on the contract and on each function again for increased dramatic effect.
...
We also searched for any known sighashes on [4byte.directory](https://www.4byte.directory/) that would clash with the ones we blocked, in order to double warn users if there are any. At that time there weren't any other know hash collisions and only `proxyAdmin` and `beacon` were actually present in the 4byte directory database.
...
- Add an existing beacon to the factory, while also providing a valid alias, beacon ownership must be be given to the factory before the call, otherwise the operation reverts with `CollectionFactory: ownership must be given to factory`, this is done by calling the `addBeacon` function.
...
- Any new collection can now be deployed by calling `deployCollection` with the alias of the beacon that will be paired with the collection and initialization arguments specific to the implementation. That's the entire initial setup process. Collection for which the available beacon/implementation are suffice, can be launched by calling `deployCollection`.
...
In order to update the implementation of one specific collection we need to have a beacon contract that points to this new implementation. Whether this beacon is a newly deployed one (via `deployBeacon`), an already existing one but modified with `updateBeaconImplementation` or a beacon added directly to the factory via `addBeacon` it is irrelevant. The only requirement is that the beacon to be tracked by the factory.

Updating the collection is done calling the `updateCollection` function.
...
If we are in a circumstance where collections are to be added to the factory, collections that are not deployed by the current instance of the factory, then we use the `addCollections` function.

This functions has several key checks:
